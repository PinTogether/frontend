...
const MapNaverDefault = () => {
  ...
  function intersectsBound(marker1: naver.maps.Marker, marker2: naver.maps.Bounds) {
  ...
  function makeClusteredMarkerList(){
    ...
    createMarkerList.forEach((marker, markerIndex) => {
      if(newClusteredMarkers[0] && marker.getMap()){
        ...
        for (let clusteredMarkerIndex = 0 ; clusteredMarkerIndex < newClusteredMarkers.length ; clusteredMarkerIndex++){
          if (intersectsBound(marker, newClusteredMarkers[clusteredMarkerIndex].bound)) {
        ...
        if(!isIntersect){
          newClusteredMarkers.push({innerMarkerList:[marker], innerMarkerId:[markerDatas[markerIndex].id], centerPoint: marker.getPosition(), bound: marker.getDrawingRect(), clusteredMarker:null, infoWindow:null});
          //marker.setMap(null);
        }
      }
    ...
    if(newClusteredMarkers[0]){
      newClusteredMarkers.forEach((ClusteredMarkerData, index)=>{
        ...
        if(newMap)
          ClusteredMarkerData.clusteredMarker.setMap(newMap); //임시
      }
      else{
