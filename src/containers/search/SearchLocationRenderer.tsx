...
export default function SearchLocationRender({ pindatas }: { pindatas: IPin[] }) {
  ...
  return (
    <section className={styles.searchPartContainer}>
      ...
      <section className={styles.searchListContainer}>
        {pindatas.map((pinData, index) => (
          index <= repeatCount && <LocationCard key={index} locationData={pinData} />
        ))}
      </section>
    </section>
  );
